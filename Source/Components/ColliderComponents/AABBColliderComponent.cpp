...
AABBColliderComponent::AABBColliderComponent(class Actor* owner, int dx, int dy, int w, int h, ColliderLayer layer, int updateOrder)
        :ColliderComponent(owner, layer, updateOrder)
...
const Vector2 AABBColliderComponent::GetMin() const
{
    return mOwner->GetPosition() - Vector2(mWidth/2.0f, mHeight/2.0f) + mOffset;
}

const Vector2 AABBColliderComponent::GetMax() const
{
    return GetMin() + Vector2((float)mWidth, (float)mHeight);
}

const Vector2 AABBColliderComponent::GetCenter() const
{
...
AABBColliderComponent::Overlap AABBColliderComponent::GetMinOverlap(AABBColliderComponent* b) const
{
    ...
    auto *a = const_cast<AABBColliderComponent*>(this);

    std::unordered_map<int, Overlap> overlaps;
    overlaps[0] = {Vector2::UnitX * (bMax.x - aMin.x), CollisionSide::Left, a, b};
    overlaps[1] = {Vector2::UnitX * (bMin.x - aMax.x), CollisionSide::Right,a, b};
    overlaps[2] = {Vector2::UnitY * (bMax.y - aMin.y), CollisionSide::Top,  a, b};
    overlaps[3] = {Vector2::UnitY * (bMin.y - aMax.y), CollisionSide::Down, a, b};
